...
public class EuropeanCentralBank {
    ...
    private Map<String, Double> fetchTable() throws Exception {
        ...
        for (int i = 0; i < cubes.getLength(); i++) {
            ...
            if (el.hasAttribute("currency")) {
                rates.put(
                        ...
                        Double.valueOf(el.getAttribute("rate"))
                );
            }
        }
        return rates;
    ...
    public double getRateEURto(String target) throws Exception {
        if (cache == null) cache = fetchTable();
        Double rate = cache.get(target.toUpperCase());
        if (rate == null)
            throw new IllegalArgumentException("Currency not in ECB list: " + target);
        return rate;
    }
}
